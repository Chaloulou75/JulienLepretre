...
<div class="max-w-sm mx-auto">
	...
	<h1 class="font-bold text-3xl text-julien-gris text-center uppercase my-4">
		...
		<form class="bg-gray-100 border-2 border-white shadow-md rounded px-8 pt-6 pb-8 mb-4 animated bounceInUp" method="POST" enctype="multipart/form-data" action="{{ route('testimonios.store') }}">
        @csrf
	        ...
	        <div class="mb-4">
	            ...
	            <textarea id="msg" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-900 leading-loose focus:outline-none focus:shadow-outline @error('msg') bg-red-dark @enderror" name="msg" value="{{ old('msg') }}" required autocomplete="msg">{{ old('msg') }}</textarea>
	        ...
	        <div class="mb-4">
	            <div class="block items-center justify-between">
	                <button type="submit" class="w-full bg-julien-gris hover:bg-julien-red text-white font-semibold text-center tracking-widest border-2 rounded border-white flex-auto py-2 px-4 m-2">
	                    {{ __('Enviar') }} &raquo;
	                </button>
	            </div>
	        </div>
