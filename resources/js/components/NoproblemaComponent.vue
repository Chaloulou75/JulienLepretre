    ...
    <div class="w-full lg:py-2 text-center text-justify mx-auto rounded overflow-hidden my-4"> 
      ...
      <transition name="custom-classes-transition" enter-active-class="animated bounceInLeft" leave-active-class="animated bounceOutRight">
        <div v-if="isOpen">
          ...
          <div class="px-6 py-2">   <!-- :class="isOpen ? 'block' : 'hidden'" -->
            <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-julien-gris mr-2">#EQUILIBRIO</span>
            <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-julien-gris mr-2">#DEPORTE</span>
          </div>
        </div>
      </transition>
    </div>
